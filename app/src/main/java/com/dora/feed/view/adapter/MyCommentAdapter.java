 ...
 */
public class MyCommentAdapter extends BaseDataBindingAdapter {
    private List<CommentBean.Data> list;

    public MyCommentAdapter(Context context, ArrayList<CommentBean.Data> datas) {
        super(context, datas);
        list = datas;
    }
    ...
    protected void onBindView(RecyclerView.ViewHolder holder, int position, Object data) {
        ...
        ((ViewHolder) holder).getBinding().setVariable(BR.bean, ((CommentBean.Data) data));
    ...
    @BindingAdapter("app:imgCommentHead")
    public static void setHeadImageUrl(ImageView view, String url) {
        x.image().bind(view,CacheUtils.getInstance().getString(LocalContents.LOGIN_USER_HEAD));
    }
